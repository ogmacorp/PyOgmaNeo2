...
predictionState = state.getPredictionCs(0)
...
for t in range(300):
    ...
    # Predict additional steps ahead
    stateAhead = state.clone()

    for i in range(stepsAhead):
        h.step(cs, [ stateAhead.getPredictionCs(0) ], stateAhead, False)

    # Retrieve the predicted column index
    predIndex = stateAhead.getPredictionCs(0)[0] # First (only in this case) input layer prediction
